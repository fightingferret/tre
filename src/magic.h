...
/*
 *   The above conditions have changed and parts are no longer valid.
 *   A sixth argument  int type  has been added for use in determining
 *   the way in which the spell is being used.  Values are defined below
 *   as STYPES.
 *     PUIOK - 23th July 2000
 */
...
bool         using_reagent    ( char_data*, obj_data* );
void         unlink_reagents  ( cast_data* );
...
#define STYPE_CASTING               0
#define STYPE_PROC_CAST             1
#define STYPE_RECITE                2
#define STYPE_ZAP                   3
#define STYPE_QUAFF                 4
#define STYPE_POTION_THROW          5
#define STYPE_DRINK                 6
#define STYPE_LIQUID_THROW          7
#define STYPE_DIP                   8
#define STYPE_FILL                  9
...
{
  if( ch == NULL ) {
    bug( "%s: Null caster unsupported.", skill_table[spell].name );
    return TRUE;
    }
...
{
  if( corpse == NULL ) {
    bug( "%s: Null corpse unsupprted", skill_table[spell].name );
    return TRUE;
    }
