...
{  
  ...
  if( change ) {
    fprintf( fp, "A request was entered at riversedge.nu to change\n");
    ...
    fprintf( fp, "A request was entered at riversedge.nu to open an\n" );
...
(char*) ctime( &current_time ) );
  ...
  sprintf( tmp,
    "/usr/lib/sendmail -friversedge@riversedge.nu -t %s < email.msg", account->new_email );
  system( tmp );
...
void nanny_acnt_request( link_data* link, char* argument )
{
  ...
  sprintf( tmp,
    "/usr/lib/sendmail -friversedge@riversedge.nu -t %s < email.msg", account->email );  
system( tmp );
