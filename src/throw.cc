...
void do_throw( char_data* ch, char* argument )
{
  ...
  if( ( victim = one_character( ch, argument, "throw at", ch->array ) ) ==
    NULL )
    return;
  ...
  if( obj->Weight( 1 ) > 10*ch->Strength( ) ) {
...
void throw_potion( char_data* ch, char_data* victim, obj_data* obj )
{
  ...
  if( throw_liquid( ch, victim, obj ) ) {
    ...
    if( !is_set( &spell_table[ obj->value[0] ].usable_flag,
      STYPE_POTION_THROW ) ) {
      send( ch, "Nothing seems to happen.\n\r" );
      }
    else {
      ( *spell_table[ obj->value[0] ].function )( ch, victim, obj,
        level, duration, STYPE_POTION_THROW );
      }
    }
...
void throw_drink_con( char_data* ch, char_data* victim, obj_data* obj )
{
  ...
  if( throw_liquid( ch, victim, obj ) && spell != -1 ) {
    ...
    if( is_set( &spell_table[spell-SPELL_FIRST].usable_flag, STYPE_LIQUID_THROW ) )
      ( *spell_table[spell-SPELL_FIRST].function )( ch, victim,
        NULL, 10, 5, STYPE_LIQUID_THROW ); 
    }
