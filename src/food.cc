...
bool eat( char_data* ch, obj_data* obj )
{
  if( obj->pIndexData->item_type == ITEM_CORPSE ) {
    ...
    if( ch->shdata->race == RACE_TROLL ) {
      ...
      if( cant_take( obj, ch ) == NULL ) {
        send( ch, "You can't eat that!\n\r" );
        return FALSE;
        }
      else if( forbidden( obj, ch ) == NULL ) {
        send( ch, "You are forbidden from eating that.\n\r" );
        return FALSE;
        }
      else if( sat_on( obj, ch ) == NULL ) {
        send( ch, "Someone is sitting on %s.\n\r", obj );
        return FALSE;
        }
      }
    else {
  ...
  switch ( obj->pIndexData->item_type ) {
    ...
    case ITEM_CORPSE:
      ...
      send( ch,
        ...
        obj );
      fsend( *ch->array,
        "%s pulls apart %s and eats it piece by piece making a disgusting sight.\n\r", ch, obj );
      break;
