  ...
  int level, int, int )
{
  ...
  if( !is_set( &species->act_flags, ACT_HAS_SKELETON ) ) {
    fsend( ch, "%s did not contain a skeleton and so trying to animate it was doomed to failure.  All that remains on the ground is a few ashes.",
      corpse );
  ...
  fsend( ch, "The skeleton of %s slowly rises from the ground and starts to follow you.", species->descr->name );

  fsend_seen( remains, "The skeleton of %s slowly rises from the ground and starts to follow %s.", species->descr->name, ch );
  ...
  int level, int, int )
{
  ...
  damage_magic( victim, ch, spell_damage( SPELL_TURN_UNDEAD, level ),
    "*the enchantment" );
  ...
  int level, int, int )
{
  ...
  damage_magic( victim, ch, spell_damage( SPELL_BANISHMENT, level ),
    "*the enchantment" );
