...
thing_data* sat_on( thing_data* thing, char_data* ch, thing_data* )
{
  obj_data*  obj;
  char_data* rch;
  ...
  for( int i = 0; i < *obj->array; i++ )
    if( ( rch = character( obj->array->list[i] ) ) != NULL )
      if( rch->pos_obj == obj && !is_set( rch->affected_by, AFF_FLOAT ) )
...
thing_data* in_use( thing_data* thing, char_data* ch, thing_data* )
{
  obj_data* obj;

  if( ( obj = object( thing ) ) != NULL
    && using_reagent( ch, obj ) )
    return NULL;
...
void select( thing_array& list, char_data* ch, bool not_self )
{
  for( int i = 0; i < list; i++ )
    list[i]->selected = ( list[i]->Seen( ch )
      && ( !not_self || list[i] != ch ) ? list[i]->number : 0 );
}  
...
void list_horse_wear( char_data* ch, thing_array* array, char_data* horse )
{
  ...
  if( ( thing = one_shown( *array ) ) != NULL ) {
    ...
    switch( thing->position ) {
      case WEAR_HORSE_BACK:
        fpage( ch, "You place %s on the back of %s.\n\r", thing, horse );
        send_seen( ch, "%s places %s on the back of %s.\n\r",
          ch, thing, horse ); 
        break;
      case WEAR_HORSE_BODY:
        fpage( ch, "You put %s on %s, protecting %s body.\n\r",
          thing, horse, horse->His_Her( ) );
        send_seen( ch, "%s puts %s on %s, protecting %s body.\n\r",
          ch, thing, horse, horse->His_Her( ) ); 
        break;
      case WEAR_HORSE_FEET:
        fpage( ch, "You place %s on the feet of %s.\n\r", thing, horse );
    ...
    if( !first_list )
      page( ch, "\n\r" ); 
    page( ch, "You put on %s:\n\r", horse );
    for( int i = 0; i < *array; i++ ) {
      thing = (obj_data*) array->list[i];
      if( thing->shown > 0 )
        page( ch, "  %s %s.\n\r", wear_verb[ thing->position ], thing );
      }
    fsend_seen( ch, "%s puts several items on %s.\n\r", ch, horse );
    }
}

/*
void act_message( char_data* ch, const char* text, 
